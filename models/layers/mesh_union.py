...
class MeshUnion:
    ...
    def rebuild_features_average(self, features, mask, target_edges):
        ...
        # 使用安全的矩阵乘法，处理CUBLAS错误
        try:
            fe = torch.matmul(features_squeezed, self.groups)
        except RuntimeError as e:
            if "CUBLAS_STATUS_EXECUTION_FAILED" in str(e):
                print("Warning: CUBLAS error in mesh_union, falling back to CPU computation")
                # 回退到CPU计算
                features_cpu = features_squeezed.cpu()
                groups_cpu = self.groups.cpu()
                fe_cpu = torch.matmul(features_cpu, groups_cpu)
                fe = fe_cpu.to(features_squeezed.device)
            else:
                raise e
